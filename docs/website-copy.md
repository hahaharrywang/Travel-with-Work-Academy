# 遠距遊牧學院官網文案（完整版）

> 最後更新：2026/04/16

---

## 1. Hero Section（首頁主視覺）

### 標題
今年五月，一起把「也許有一天」變成「**我也正在路上**」

### 副標題
不停薪、不 all-in，五個月找到適合你的遠距路徑。

### 適合誰
不管你是想找到一份遠距工作、用接案或內容創作多一條收入線，還是只是覺得「人生不該只有這樣」。

### 膠囊標籤
- 遠端上班 × 自由接案
- 不停薪開始
- 5 個月有節奏
- 可展示成果

### 三大賣點
1. 遠端上班 × 接案雙軌並行，可雙修、也可單線
2. 履歷、作品集、個人頁面，不再只是想過，可展示
3. 整合線下小聚、國內外遊牧之旅、國際生態系入口

### 社會證明
至今已累積 **300+** 學員、**1,800+** 線下社群參與人次

### CTA
- 查看課程介紹
- 報名線上講座

### 引導文案（手機版）
AI時代下已經不缺資訊，你需要的是一個驗證過的決策指引＆環境，在未來五個月裡，有人陪你一起試、一起走、一起調整方向。

---

## 2. 正在尋找「下一步」的你

### 標題
正在尋找「下一步」的你

### 描述
你不一定已經決定要不要辭職、要不要成為全職 Nomad。
但你大概知道——現在的生活節奏，好像不是你想一直過下去的版本。

**你可能會在這幾種狀態裡，看見自己的影子：**

### 三種狀態卡片

#### 1. 悶在穩定工作裡，覺得人生應該還有別的可能
工作不算差，但每天通勤、開會、等週末，日子過得到，卻也看得到天花板。偶爾滑到別人遠距工作的限動，心裡有一點什麼說不清楚——不是要逃，而是開始覺得自己值得更多選擇。

#### 2. 其實動過了，但什麼都碰、什麼都沒做完
你不是沒努力。經營過自媒體、投過接案平台、改過 LinkedIn，認真上過幾堂課。但做了一輪之後，沒有一個作品集能拿出來、沒有穩定的案子——有點累了，不是累在努力，而是累在一直重新開始。

#### 3. 被故事吸引了很多次，但從來沒跨出去
看過很多人邊旅行邊工作的分享，每次都有點心動，但總覺得那些人的起點跟自己差太遠。你不是不想，只是不知道「從我現在的狀態」到底能從哪裡開始——而每次猶豫完，又回到原本的日常。

---

## 3. 痛點區塊（Pain Points）

### 標題
不是你不努力，而是拼圖還有缺

### 三大痛點

#### 方向斷裂
想過很多版本，但每條路都沒真的走下去。

#### 方法斷裂
買課、看影片、存筆記，但不知道今天該做哪一步。

#### 同伴斷裂
身邊沒人在走這條路，久了你開始懷疑自己：好像不是那種「夠格」開始的人。

### 結語
AI時代下已經不缺資訊，你需要的是一個驗證過的決策指引＆環境，**在未來五個月裡，有人陪你一起試、一起走、一起調整方向。**

---

## 4. 學院三大特色

### 標題
學院三大特色，讓行動成為習慣

### 副標題
不只是學習新知
雙軌資源、行動任務節奏，和一套幫你持續走下去的支持結構。

---

### 特色一：雙軌資源

**簡述：** 這裡不是要你馬上做對選擇，而是可以開始行動起來，讓行動告訴你答案。

**詳細說明：**
- **接案線：** 幫你釐清主題定位，做出接案作品集，建立基本的市場研究、內容策略與變現思維。
- **遠端上班線：** 帶你看懂遠端求職市場，整理履歷與 LinkedIn，練習求職信、面試與獵頭溝通。
- 你可以雙線並進，更快探索兩種可能；也可以先選一條主線，保留之後調整的彈性。
- 這裡不是要你現在就選到最完美，而是幫你在未來五個月裡，用更低風險的方式開始試出方向。

---

### 特色二：行動任務節奏

**簡述：** 每堂課都有做得到的行動任務，拆成模板和小步驟，不再只聽懂卻做不出來。

**詳細說明：**
- 每堂課都會對應一個做得到的任務：目標設定、定位整理、AI 加速產出、履歷更新、求職信撰寫、內容企劃、作品集頁面等。
- 任務不是丟給你自己想，而是會搭配學習單、模板、範例與拆解步驟，讓你知道今天到底該先做哪一步。
- 想走更快，也可以搭配實作型工作坊，例如短影音剪輯、Coffee Chat、Vibe Coding、工作英語等，針對你當下最需要補強的地方加速前進。
- 學院的重點不是讓你「學過」，而是讓你完成、留下、可以被看見。

---

### 特色三：支持結構

**簡述：** 你不需要靠意志力一個人硬撐，這裡有一整套比較走得下去的支持系統。

**詳細說明：**
- 從 Skool 共學空間、Line 群、線上同學會、校友 LinkedIn 群，到不定期線下小聚，一路上都有人可以交流、回報進度、彼此提醒。
- 真正難的通常不是開始，而是做幾週之後開始懷疑自己、忙起來就斷掉。
- 所以我們把支持設計進節奏裡：固定交流、成果分享、校長 AMA、講師 QA、共創與復盤，不是只提供陪伴感，而是幫你提高真的走完的機率。
- 這也是為什麼很多人不是因為「更有天份」才做到，而是因為在卡住時，還有人和結構把他拉回來。

---

## 5. 學員真實成果

### 標題
學員真實成果

### 副標題
因為加入了學院，開始有了不一樣。

### 描述
在遠距遊牧學院，我們不是只上課看影片，而是完成一個個具體行動：
有人從沒有作品集，到做出第一個 side project；有人開啟接案變現；有人第一次帶著工作出國旅居，找到了自己想過的生活樣子。

---

### 案例 A｜從 0 到 1 的作品集突破

**身份：** 科技公司 PM，從沒公開過自己作品

**標籤：** #遠距遊牧學院

**引言：**
> 我第一次可以很有底氣地，把作品集網站連結，貼在履歷跟訊息裡，不再只是說自己「會盡力完成工作」，而是讓成果自己說話。

**完整心得：**
加入學院之前，我在科技公司當了三年 PM，工作穩定、不算討厭，但每天就是通勤、開會、交報告。偶爾看到有人在海外遠距工作的分享，心裡會想「也許我也可以」，但想到自己完全沒有作品集——頂多只有幾個零散的 side project 與想法——每次想到要投履歷或嘗試接案，就卡在同一句話：「我好像沒有什麼可以拿得出來。」

後來在學院的實作任務裡，我跟著專業框架一步一步整理自己的過去經驗，完成了第一個真正能「被展示」的個人專案：把專案背景寫清楚、我負責做了什麼、最後的產出與成效是什麼，並且整理成一份正式的作品集頁面，後來透過 vibe coding 工作坊，甚至做出了個人網頁。

**現在狀態：**
- ✔ 已完成可投遞的作品集與個人網頁
- ✔ 已開始對外投遞遠端職缺
- ✔ 清楚知道下一個 90 天要做什麼

---

### 案例 B｜開啟接案變現之路

**身份：** 邊上班邊經營 IG 的設計人

**標籤：** #遠距遊牧學院 #接案線

**引言：**
> 原來真的有人願意為我的內容付費，第一次打從心底覺得接案是一條可以慢慢累積變現的路，我會永遠記住這第一份線上收入。

**完整心得：**
加入學院之前，我邊上班邊經營 IG，其實已經動了好一陣子。但做了半年，什麼都碰了一點，卻什麼都沒有真正做完——沒有穩定變現、也不知道下一步要加強哪裡。我一直有在發文、分享想法，但從來沒有收集過電子報名單，也不知道要怎麼把內容變成真正的付費產品。

在學院的課後任務裡，我跟著步驟做了人生第一個「電子報引導頁」：先想清楚我想服務的讀者是誰、我可以承諾對方獲得什麼，接著把訂閱流程設定好，開始穩定邀請 IG／社團的追蹤者留下 email。

兩週後，我收到了我的第一筆線上訂閱收入：30元，雖然不多，但收到當下我非常興奮。

**現在狀態：**
- ✔ 已建立電子報訂閱頁與自動化流程
- ✔ 已獲得第一筆線上訂閱收入
- ✔ 持續累積付費內容產品線

---

### 案例 C｜第一次邊工作邊旅居

**身份：** 在台北工作 7 年、第一次帶著工作出國

**標籤：** #Journey #邊工作邊旅行

**引言：**
> 旅居不再只是想像中的夢，而是一個可以被規劃、也可以被重複的生活選項。這次之後我才明白：它只是需要經驗與準備，透過社群夥伴的幫助下，其實沒這麼困難。

**完整心得：**
一開始我只是在網路上聽大家分享遊牧故事，心裡想著：「好羨慕，但應該不會輪到我吧。」

因緣際會之下我跟著遊牧社群一起去了福岡，為自己安排了一次「一週旅居實驗」：白天大多時間還是要工作，不同的是，下班之後我有了新的選擇：晚上在異國街道散步、吃道地豚骨拉麵、探索城市。回到台灣後，我更清楚自己適合什麼樣的節奏——怎麼在旅途中面對未知、如何在探索與自律間取捨。

**現在狀態：**
- ✔ 已完成第一次海外旅居工作實驗
- ✔ 已建立可重複的旅居工作節奏
- ✔ 正在規劃下一趟 30 天長期旅居計畫

---

### 案例 D｜從「一直在想」到真的動起來

**身份：** 行銷公司企劃，工作第四年，每天都在幫別人做內容

**標籤：** #接案

**引言：**
> 以前我每次看到「遠距工作」的貼文都會存起來，但存了一整個資料夾也沒用。學院讓我第一次不是在收藏別人的故事，而是在寫自己的。

**完整心得：**
加入學院之前，我每次看到「遠距工作」的貼文都會存起來，但存了一整個資料夾也沒用。我不是沒有想法，而是不知道從哪一步開始——看了很多人的分享，但總覺得他們的起點跟我不一樣。

進學院之後，我第一次不是在收藏別人的故事，而是在寫自己的。我跟著任務完成了第一版服務定位、做了三次潛在客戶訪談、整理出一個真的可以對外講的 Offer 頁面。過程中我才發現，原來我不是沒有能力，只是從來沒有人幫我把這些能力「變成一個可以被看見的東西」。

**現在狀態：**
- ✔ 已完成個人服務定位與第一版 Offer 頁面
- ✔ 完成 3 次潛在客戶市場調查
- ✔ 建立每週固定產出節奏，不再只是「存貼文」

---

## 6. 遊牧生態系入口

### 標題
你踏入的，不只是這五個月的課程
而是一整個遊牧生態系的入口

### 副標題
從線上到線下｜從台灣到國際｜從知識到行動
讓學習不只停在教室裡，而是延伸到更多真實的人、場域與機會。

---

### 三大延伸資源

#### 國際鏈結
接軌海外遊牧社群與活動資訊，持續看見國際趨勢、海外案例與遊牧者／領袖訪談。

#### 線下場域
從定期小聚到遊牧啟發旅程，讓你有機會在真實場域裡認識同路人、潛在合作夥伴與更多生活方式的可能。

#### 工作坊
除了正課之外，還能透過主題工作坊與實作型活動，把知識更快轉成行動與產出。

---

### 兩個入口

#### Lifestyle 社群入口
@digitalnomadstaiwan
看見遊牧生活方式、每月聚會與啟發旅程

#### 職涯成長入口
@twnomadacademy
追蹤講座、工作坊、訪談與更多職涯成長內容

---

### CTA
還不確定適不適合？先看免費講座／回放，再決定要不要加入。

---

## 7. 講師陣容

### 標題
你的路線，不會只有一位老師在陪你走

### 描述
這堂學院不是把所有主題塞給同一個講師，
而是找了一群真的在路上走的人，一起陪你打底、選方向、走路線。

---

### A 線｜接案線路導師
帶你建立個人品牌、經營內容、從零開始接案變現

- **工具王阿璋** - 『阿璋遊牧』電子報創辦人、數位遊牧陪跑計劃創辦人、IP 經營者
- **林佳 Zoe** - 每日e錠 共同創辦人
- **三分鐘 Lynn** - 三分鐘行銷在學中、內容顧問
- **西打藍** - 自由文字接案者｜一人公司創辦人

---

### B 線｜遠端上班線路導師
帶你建立國際職涯視野、遠端求職策略、跨國人脈經營

- **許詮** - 前 TikTok 子公司總經理、前阿里巴巴子公司副總、XChange創辦人、33 歲退休旅居峇里島
- **Shelley** - ADPList 2025 Top 50 Global Mentor，LinkedIn 個人品牌術
- **讀者太太** - 英國職涯教練、「女力學院」《人脈力》講師
- **Emilia** - 高階跨國獵頭，獵頭揭密談薪技巧與職涯躍升策略

---

### 共同必修
打底知識變現、AI 工具、財務思維、人生 SOP 的核心講師

- **林上哲** - nuva 創辦人，致力推動 AI 教育與應用，與 NVIDIA 官方合作擔任 AI 與 AIGC 講師
- **鮪魚** - 專注於知識變現與內容創新，協助超過百位講師完成課程開發，累積銷售額突破 3 億
- **Joyce Weng** - 過去為記者的她，跳脫傳統、成功於海外轉型遠全遠距工作
- **Angela Feng** - NESS Wellness 共同創辦人｜國際演講者｜創投顧問｜亞洲品牌策略與社群經營專家

---

## 8. 免費講座區塊

### 標題
還不確定要不要加入？先聽聽校長說明會分享

### 描述
先看懂：你適不適合、偏哪條路、這 5 個月會怎麼走。

### CTA
- 報名免費講座、認識講師
- 其他免費學習資源
- 訂閱隨時收到最新活動提醒

---

## 9. 如何加入（定價區塊）

### 標題
如何加入

### 描述
先選一個最適合你現在狀態的起跑方式。
不一定要一次走到最完整，但你可以先決定：要專心走一條主線，還是用五個月更快打開兩種可能。

### 價格調漲說明
隨著更多資訊釋出，**每週日說明會後午夜調漲**價格

---

### 方案一：接案線路
適合想要經營個人品牌、接案或創作者

**包含內容：**
- 接案路線必修課程 X4
- 共同必修課程 X4
- 學院成長節奏(開學畢業典禮、同學交流會、復盤、期末 DemoDay) X10
- 共創專案參與機會
- 終身校友資格 (人脈群組、優先資訊＆專屬折扣)

---

### 方案二：雙線並進（推薦）
同時探索兩條路線，適合最怕選錯、也想在五個月內更快看見答案的人

**包含內容：**
- 雙路線必修課程 X8
- 共同必修課程 X4
- 學院成長節奏(開學畢業典禮、同學交流會、復盤、期末 DemoDay) X10
- 共創專案參與機會
- 終身校友資格 (人脈群組、優先資訊＆專屬折扣)

---

### 方案三：遠端上班線路
想找到一份遠端工作、時間地點自由的你

**包含內容：**
- 遠端上班路線必修課程 X4
- 共同必修課程 X4
- 學院成長節奏(開學畢業典禮、同學交流會、復盤、期末 DemoDay) X10
- 共創專案參與機會
- 終身校友資格 (人脈群組、優先資訊＆專屬折扣)

---

### 重要說明
學院方案以「課程＋社群＋校友資格」為主；多數線下活動／工作坊／旅程為選配加購，校友皆享有專屬折扣。

---

## 10. 常見問題 FAQ

### 適合對象

#### Q: 這堂學院適合什麼樣的人？我現在只是在上班，可以報名嗎？
當然可以！學院就是為了「想探索多元可能」的人設計的。你如果正在上班，完全不需要先離職，也不需要有任何接案或遠距經驗。只要你願意在未來五個月裡，每週騰出 2-4 小時來學習和行動，這裡就適合你。

#### Q: 英文不好、不會寫程式，可以嗎？
可以。我們的課程主要用中文進行，雖然英文能力＆程式能力在遠距遊牧之路上很加分，但絕對不是會阻止你開始的理由。選修課程中有「工作英文」和「AI vibe coding 等工具應用」，會幫助你從零建立方向與技能。最重要的是「願意以開放的心開始」，而不是「已經會」。

---

### 時間與節奏

#### Q: 學習的節奏大概是怎麼安排的？會不會太硬？
這是一套五個月、四階段的行動節奏。前段會先幫你看清方向、建立門面；中段開始機會與轉化；後段則更重視復盤、永續、共創與成果發表。

每週主課約 120 分鐘，搭配作業與社群節奏，整體設計是讓有正職的人也跟得上。

#### Q: 我時間很不固定，有錄影可以回看嗎？作業一定要每週交嗎？
所有課程都會錄影，放在 Skool 社群讓你隨時回看。作業有建議繳交時間，但我們更鼓勵「完成比完美重要」——如果某段時期真的忙不過來，可以先跟上進度，之後再補。我們會有基本的及格門檻，但不會逼你每週都交滿。

---

### 內容與線路

#### Q: 接案線路與遠端上班線路有什麼差別？
**接案線路：** 適合想透過內容創作與個人品牌，建立「可變現的影響力」與收入來源的人。重點會放在：定位、內容策略、變現路徑、拿到第一個案子。

**遠端上班線路：** 適合想找到一份可遠距的正職／長期合作的人。重點會放在：履歷與作品呈現、求職策略、遠距職缺管道、面試準備。

#### Q: 報名時先選了一條主線，可以中途換線嗎？
可以。第一個月的 2、3 週會有試上另一條線起步課的機會：
- Week 2：接案變現地圖＆目標設定
- Week 3：遠端上班職涯藍圖＆目標設定

**第 1 個月結束前可申請 1 次調整**
你可以選擇**換線**（單路線 A → 單路線 B），或**加購升級成雙線並進**（單路線 → 雙路線）。

※ 申請截止後，後續路線將鎖定，以確保後續社群學習體驗品質。

---

### 結業與權益

#### Q: 結業後還能看回放嗎？
你會永久保留自己買過的課程回放與資源。同一個 Skool 社群也會持續保留，畢業後不用搬家，仍可在社群中交流（但「當屆專區」僅當屆可見）。

---

## 11. 課程行事曆（四階段）

### 階段一：藍圖與目標（5 月）
**先知道你要往哪裡走**
先搞清楚你想往哪裡去。不是空想自由，而是把你接下來幾個月要走的主線、目標與節奏定出來。

---

#### 遠端上班線 - 階段一內容

**標題：** 先看懂，這條路長什麼樣

**描述：** 你會釐清自己想要的工作型態、適合的職缺方向，以及接下來幾個月要集中火力的目標。

**你會得到：**
- 全局認識：拆解遠距工作市場現況，認識不同職位類型與聘僱模式。
- 安全防線：識別求職詐騙陷阱，掌握正規公司的標準應徵流程。
- 精準定位：完成遠距適配度自評，設定具體、可追蹤的求職目標。

**對應課程：** 開學典禮＆遠距遊牧概論、遠端上班職涯藍圖＆目標設定

---

#### 接案線 - 階段一內容

**標題：** 先不要急著發內容或開服務

**描述：** 這一階段會先幫你看懂接案與變現的幾種可能，找到適合自己的起跑方式。

**你會得到：**
- 更清楚的接案方向與目標
- 對自己想做的主題、服務與變現路徑有初步輪廓
- 不再只是模糊地說「我也想接案」

**對應課程：** 開學典禮＆遠距遊牧概論、接案變現地圖＆目標設定

---

| 週次 | 課程 | 路線 | 講師 |
|------|------|------|------|
| Week 1 | 開學典禮 ＆ 遠距遊牧概論 | 全體共同 | 校長哈利 |
| Week 2 | 接案變現地圖 & 目標設定 | 接案線 | 西打藍 |
| Week 3 | 遠端上班職涯藍圖 & 目標設定 | 遠端上班線 | 喬 |
| Week 4 | 交流／成果發表（月末） | 全體共同 | 校長哈利 |

---

### 階段二：定位與門面（6 月）
**把你整理成別人看得懂的樣子**
把你整理成別人看得懂、也願意相信的樣子。你的經驗、能力、價值主張，不再只是你自己知道。

---

#### 遠端上班線 - 階段二內容

**標題：** 把價值變成「更容易被看懂」

**描述：** 你會重新整理自己的經歷、優勢與故事，並把它們放進 LinkedIn 與對外呈現裡。

**你會得到：**
- 能力轉譯：盤點個人經歷，將實體協作經驗轉化為「遠距協作證據」。
- 關鍵字對焦：學習 JD 拆解術，抓出核心需求並精準對應至個人履歷。
- 數位門面：掌握 LinkedIn 搜尋引擎邏輯 (SEO)，優化檔案以提升被搜到的機率。

**對應課程：** 可持續的自由：身心靈平衡的遠距人生 SOP、Linkedin 經營全攻略

---

#### 接案線 - 階段二內容

**標題：** 讓你的價值開始有形

**描述：** 你會整理自己的定位、服務、方案與對外說法，讓別人開始看得懂你可以幫他解決什麼問題。

**你會得到：**
- 更清楚的定位與受眾輪廓
- 第一版 offer / 方案定價想法
- 比較像「可以接案的人」的對外門面

**對應課程：** 可持續的自由：身心靈平衡的遠距人生 SOP、定位、方案定價 Offer

---

| 週次 | 課程 | 路線 | 講師 |
|------|------|------|------|
| Week 1 | 可持續的自由：身心靈平衡的遠距人生 SOP | 全體共同 | Angela Feng |
| Week 2 | 定位、方案定價 Offer | 接案線 | 老 K |
| Week 3 | Linkedin 經營全攻略 | 遠端上班線 | 謝雅筑 Ally |
| Week 4 | 交流／成果發表（月末）＋團體 QA | 全體共同 | 校長哈利 |

---

### 階段三：機會與轉化（7 月）
**開始讓曝光、投遞與合作變成機會**
開始讓機會真的發生。不管是投遞、曝光、內容、獲客、面試還是成交，這一階段會讓你開始對外出擊。

---

#### 遠端上班線 - 階段三內容

**標題：** 真正開始出擊

**描述：** 這一階段會進入 AI 工具應用、履歷、求職信、面試與談薪，把「我想投」變成「我真的可以去爭取」。

**你會得到：**
- 精準出擊：了解各大遠距求職平台特性，規劃有節奏的主動投遞策略。
- 臨場應對：練習用 STAR 架構回應遠距面試常見問題，展現專業與穩定度。
- 關鍵談判：學會薪資談判心法與 Offer 條款判讀，爭取合理待遇與保障。

**對應課程：** Focus on Your True Value 讓 AI 成為你的實習生：從對話到自動化的第一個流程、履歷、求職信秘笈、面試談薪、自媒體變現藍圖

---

#### 接案線 - 階段三內容

**標題：** 開始對外被看見，也開始學會讓曝光變成機會

**描述：** 這一階段會進入 AI 工具、內容策略、社群獲客、收入藍圖，讓你不只是產出內容，而是更知道怎麼靠近合作與收入。

**你會得到：**
- 更完整的內容方向與獲客思路
- 一套更有策略的對外曝光方式
- 從「有內容」走向「有機會成交」

**對應課程：** Focus on Your True Value 讓 AI 成為你的實習生：從對話到自動化的第一個流程、社群獲客漏斗＆內容、自媒體變現藍圖

---

| 週次 | 課程 | 路線 | 講師 |
|------|------|------|------|
| Week 1 | Focus on Your True Value 讓 AI 成為你的實習生 | 全體共同 | 林上哲 |
| Week 2 | 社群獲客漏斗＆內容 | 接案線 | 三分鐘 Lynn |
| Week 3 | 履歷、求職信秘笈、面試談薪 | 遠端上班線 | 講師確認中 |
| Week 4 | 自媒體變現藍圖 | 全體共同 | 工具王阿璋 |
| Week 5 | 交流／成果發表（月末） | 全體共同 | 校長哈利 |

---

### 階段四：永續（8–9 月）
**找到工作不是終點，走得久才是**
從「找到機會」到「持續經營」
把一次嘗試，變成能走下去的節奏。不是短暫熱血，而是學會留任、合作、溝通、財務與長期累積。

---

#### 遠端上班線 - 階段四內容

**標題：** 找到工作不是終點，走得久才是

**描述：** 這一階段會帶你思考留任、溝通、旅居財務、節奏管理與成果整合，讓遠端工作不是短期幻想，而是更能持續的生活方式。

**你會得到：**
- 信任累積：理解「成果導向」思維，學會在遠端環境快速建立專業信任。
- 協作升級：養成非同步溝通與文件化習慣，有效與跨時區團隊合作。
- 永續發展：規劃個人長期競爭力路徑，讓遠距工作成為可持續的職涯選擇。

**對應課程：** 學習復盤＆目標調整、留任策略＆溝通、旅居財務課程、共創專案／期末成果發表／結業典禮

---

#### 接案線 - 階段四內容

**標題：** 接到案，不代表能長久；能合作下去，才會真的穩

**描述：** 這一階段會帶你思考合作關係、交付節奏、財務安排與長期累積，讓接案不是偶發，而是能延續。

**你會得到：**
- 更成熟的合作與溝通思維
- 對長期接案節奏更清楚的想像
- 一套可以走出學院、繼續使用的個人運作方式

**對應課程：** 學習復盤＆目標調整、接案的永續與合作、旅居財務課程、共創專案／期末成果發表／結業典禮

---

| 週次 | 課程 | 路線 | 講師 |
|------|------|------|------|
| 8 月 Week 1 | 學習復盤＆目標調整 | 全體共同 | 校長哈利 |
| 8 月 Week 2 | 接案的永續與合作 | 接案線 | 林佳 Zoe |
| 8 月 Week 3 | 留任策略 ＆ 溝通 | 遠端上班線 | Vivinee |
| 8 月 Week 4 | 交流／成果發表（月末）＋團體 QA | 全體共同 | 校長哈利 |
| 9 月 Week 1 | 旅居財務課程 | 全體共同 | Demi |
| 9 月 Week 2 | 共創專案大會議 | 全體共同 | 校長哈利 |
| 9 月 Week 3 | 彈性課程 | 全體共同 | 校長哈利 |
| 9 月 Week 4 | 全體期末成果發表會＋學員分享＋未來機會銜接 | 全體共同 | 校長哈利 |
| 9 月 Week 5 | 結業典禮 | 全體共同 | 校長哈利 |

---

## 12. 「我還不確定」Tab 內容

### 標題
先開始，比一直想更快靠近答案

### 說明
很多人一開始不是沒興趣，而是卡在：
- 我到底比較適合遠端上班，還是接案？
- 如果選錯了怎麼辦？
- 我現在還沒很確定，現在加入會不會太早？

這很正常。因為你缺的不是再想一輪，而是一個可以安全試過一輪的環境。

### 決策彈性
**這裡給你的，不是壓力，而是決策彈性**
- 你可以先選一條主線開始
- 單線的正式學員，第 2、3 週會有試上另一條路起步課的機會
- 在第一個月內，你保有換線 / 升級雙軌的彈性

### 試上資訊
- Week 2：接案變現地圖 ＆ 目標設定
- Week 3：遠端上班職涯藍圖 ＆ 目標設定

### 結語
你不需要一次就在人生的選擇題選到最正確的答案。你只需要先進到一個，能幫你更快看見答案的行動節奏裡，先開始就好。

猶豫不決的時間，才是一去不復返的成本。

---

*文案整理完成 - 2026/04/16*
